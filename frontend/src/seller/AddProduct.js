...
import { createProduct, getAllCategories, getSellersProduct } from './helper/adminapicall';
...
const AddProduct = () => {
  ...
  const { name, description, price, quantity, image, categories, category, loading,
    createdProduct, error, getaRedirect, formData, success, errorDetails, seller
  } = values;
    ...
    if (success) {
      return (
        <Modal show={success} onHide={handleClose}>
          ...
          <Modal.Body>Woohoo, Your product {createdProduct} has been added successfully !</Modal.Body>
          <Modal.Footer>
  ...
  const preLoad = () => {
    getAllCategories().then(data => {
      ...
      .catch(err => setValues({ ...values, errorDetails: err }))

    getSellersProduct(sellerData._id, sellerData.token)
      .then(data => {
        if (data.error) {
          setValues({ ...values, error: true, })
  ...
  const onSubmit = event => {
    ...
    createProduct(sellerData._id, sellerData.token, formData)
      .then(data => {
        if (data.error) {
  ...
  return (
    <div className="row" >
      ...
      <div className="col-5 offset-2">
        <form >
          <br />
          <center><h2 className="form-group">Add Product</h2> </center>
          <br />
